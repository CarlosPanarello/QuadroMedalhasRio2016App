...
import com.carlospanarello.quadromedalhasrio2016.adapter.MedalhasPaisAdapter;
...
public class MedalhasPais extends AppCompatActivity {

    ListView mainListView;
    MedalhasPaisAdapter adapter;
    ModelHelper db;
    String codigoPais ="";
    String nomePais ="";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_medalhas_pais);

        Intent intent = getIntent();
        codigoPais = (String) intent.getSerializableExtra(Constantes.CODIGO_PAIS);
        nomePais = (String) intent.getSerializableExtra(Constantes.NOME_PAIS);

        ImageView identificaPais = (ImageView) findViewById(R.id.countryFlag);
        atribuiImagem(identificaPais,codigoPais);
    ...
    private class HttpRequestTask extends AsyncTask<Void, Void, ItemMedalhasPais[]> {
        @Override
        protected  ItemMedalhasPais[] doInBackground(Void... params) {
            try {
                if(codigoPais.isEmpty()){
                    return null;
                }

                final String url = Constantes.END_BUSCA_MEDALHAS_PAIS+codigoPais;
                RestTemplate restTemplate = new RestTemplate();
                restTemplate.getMessageConverters().add(new MappingJackson2HttpMessageConverter());
                return restTemplate.getForObject(url, ItemMedalhasPais[].class);
            } catch (Exception e) {
                Log.e("MedalhaPaisActivity", e.getMessage(), e);
            }
    ...
    private void populaLista(){
        ...
        List<ItemMedalhasPais> listaMedalhas =  db.getMedalhasPais(codigoPais);

        if((listaMedalhas == null || listaMedalhas.size() <=0) && executouPesquisa){

            AlertDialog alertDialog = new AlertDialog.Builder(MedalhasPais.this).create();
        ...
        mainListView = (ListView) findViewById(R.id.listView_Medalhas_Pais);

        adapter = new MedalhasPaisAdapter(this, R.layout.item_medalha_pais, listaMedalhas);
    ...
    private void atribuiImagem(ImageView imgFlag,String country){
        switch(country) {
            case "AFG":
